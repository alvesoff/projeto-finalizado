  ...
  addSubmit() {
      const handleSubmit = (event) => {
          ...
          if (this.formValidate(this.textarea.value)) {
              ...
              const userName = localStorage.getItem('usuario') || 'Usuário Anônimo'; // Pega o nome do usuário do localStorage
              newPost.innerHTML = `
...
const postForm = new FormPost("formPost", "textarea", "posts");
  ...
  document.querySelector('.form-login').addEventListener('submit', function(event) {
    ...
    if (email === storedEmail && senha === storedSenha) {
        ...
        atualizarNomeUsuario(storedNome);
...
function atualizarNomeUsuario(nome) {
    if (nome) {
        ...
        nomeUsuario.textContent = nome; // Atualiza o nome do usuário na interface
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    atualizarNomeUsuario(nome); // Garante que o nome já esteja atualizado ao carregar a página
